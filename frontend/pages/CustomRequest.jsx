...
import { DollarSign, MapPin, Archive, Phone, User } from 'lucide-react';
...
const CustomRequests = () => {
        ...
        try {
            ...
            if (res.ok) {
                setLeads(leads.map(item =>
                    item.lead.id === leadId ? { ...item, lead: { ...item.lead, status } } : item
                ));
    ...
    return (
        <div style={{ padding: '2rem' }}>
            ...
            {successMessage && (
                <div style={{ padding: '1rem', background: '#dcfce7', color: '#166534', borderRadius: '8px', marginBottom: '1rem' }}>{successMessage}</div>
            )}
                ...
                {[
                    ...
                    <button key={tab.id} onClick={() => setActiveFilter(tab.id)}
                        ...
                        <span style={{ marginRight: '0.5rem', fontSize: '0.8rem', opacity: 0.8 }}>
                            {tab.id === 'all' ? leads.length : (tab.id === 'NEW'
                                ? leads.filter(l => l.lead.status === 'NEW' || l.lead.status === 'SENT').length
                                : leads.filter(l => l.lead.status === tab.id).length)}
                        </span>
                    </button>
                ))}
            </div>
                            ...
                            <div key={l.id} style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                                    <div>
                                        ...
                                        <p style={{ color: '#475569', maxWidth: '600px', lineHeight: '1.6' }}>{l.description}</p>
                                ...
                                <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid #f1f5f9', display: 'flex', flexWrap: 'wrap', gap: '2rem', alignItems: 'center', justifyContent: 'space-between' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.75rem' }}>
                                        <button onClick={() => updateStatus(l.id, 'CONFIRMED')} style={{ padding: '0.6rem 1rem', borderRadius: '8px', border: '1px solid #16a34a', background: l.status === 'CONFIRMED' ? '#16a34a' : 'white', color: l.status === 'CONFIRMED' ? 'white' : '#16a34a', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.9rem' }}>✅ تم الاتفاق</button>
                                        <button onClick={() => updateStatus(l.id, 'NO_RESPONSE')} style={{ padding: '0.6rem 1rem', borderRadius: '8px', border: '1px solid #eab308', background: l.status === 'NO_RESPONSE' ? '#eab308' : 'white', color: l.status === 'NO_RESPONSE' ? 'white' : '#ca8a04', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.9rem' }}>📵 لم يرد</button>
                                        <button onClick={() => updateStatus(l.id, 'CANCELLED')} style={{ padding: '0.6rem 1rem', borderRadius: '8px', border: '1px solid #ef4444', background: l.status === 'CANCELLED' ? '#ef4444' : 'white', color: l.status === 'CANCELLED' ? 'white' : '#ef4444', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.9rem' }}>❌ ملغاة</button>
                                    </div>
                                </div>
                            </div>
                        );
